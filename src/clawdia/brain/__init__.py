...
class Brain:
    ...
    def __init__(self, model: str = "openrouter:anthropic/claude-haiku-4.5", ir: IRController | None = None):
        self._model = model
        self._ir = ir
        self.agent = create_agent(model=model, ir=ir)

    def reload_commands(self) -> None:
        """Rebuild the agent with current IR commands."""
        self.agent = create_agent(model=self._model, ir=self._ir)
